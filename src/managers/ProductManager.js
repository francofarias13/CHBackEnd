    ...
    async insertOne(data, file) {
        try {
            ...
            const product = {
                ...
                stock: Number(stock),
                category,
                thumbnails: file?.filename ? [file.filename] : [],
            };
    ...
    async updateOneById(id, data, file) {
        try {
            ...
            const product = {
                ...
                status: status != null ? convertToBoolean(status) : productFound.status,
                stock: stock ? Number(stock) : productFound.stock,
                category: category || productFound.category,
                thumbnails: newThumbnail ? [newThumbnail] : productFound.thumbnails,
            };
